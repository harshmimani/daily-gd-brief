...
GEMINI_MODELS = list(dict.fromkeys([   # tried in order until one works
...
class GeminiClient:
    ...
    def generate_json(self, prompt: str):
        ...
        if not self.enabled:
            return None
        models = [self.model_used] if self.model_used else GEMINI_MODELS
        for model in models:
            for attempt in range(GEMINI_MAX_RETRIES):
                ...
                if r.status_code in (404, 400, 403):
                    log(f"  Gemini {model} unavailable (HTTP {r.status_code}); trying next model")
                    break
